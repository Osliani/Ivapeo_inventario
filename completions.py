...
class Completions:
    ...
    def __init__(self, messages, model = "gpt-4o-mini", tools = [], functions = {}, tool_choice = "auto", response_format = False):
    ...
    def submit_message(self, message, user_number):
        if message:
        ...
        if response.choices[0].message.tool_calls:
            return self.tool_calls(response, user_number)
    ...
    def tool_calls(self, response, user_number):
        print("Tool calls!")
        ...
        for tool_call in response.choices[0].message.tool_calls:
            ...
            function_response = function_to_call(**function_args, user_number = user_number)
            self.messages.append(
                {
        ...
        return self.submit_message(self, False, user_number)
...
¿Te gustaría saber más sobre alguno de estos servicios específicos? ¡Estoy aquí para ayudarte! 💖
    ...
    ans, ok = bot.submit_message(msg, "52045846")
